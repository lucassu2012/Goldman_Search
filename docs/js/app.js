 ...
 * Fully client-side: calls GSAnalyzer directly, no backend needed.
 */
(function () {
    "use strict";
    let selectedSectors = [];
    ...
    // Form submit
    $("#screenForm").addEventListener("submit", e => { e.preventDefault(); doScreen(); });
    ...
    function doScreen() {
        ...
        const msgs = ["正在扫描市场数据...","获取实时行情...","计算估值指标...","分析收入增长...","评估竞争护城河...","生成风险评级...","计算目标价...","生成报告..."];
        let mi = 0;
        const iv = setInterval(() => { mi = (mi+1)%msgs.length; $("#loadingText").textContent = msgs[mi]; pf.style.width = Math.min(10+mi*12,90)+"%"; }, 400);
        ...
        setTimeout(() => {
            ...
            const results = runScreening(opts);
            ...
            setTimeout(() => {
                renderReportMeta(reportId, reportDate, opts, results.length);
        ...
        }, 1800); // simulate analysis time
    }

    function renderReportMeta(id, date, opts, count) {
    ...
    function renderTop3(results) {
        $("#top3List").innerHTML = results.slice(0,3).map((s,i) => `<div class="top3-item">
            <span class="top3-rank">${i+1}</span><span class="top3-ticker">${s.ticker}</span>
    ...
    function renderSummaryTable(results) {
        ...
        tb.innerHTML = results.map((s,i) => `<tr data-idx="${i}">
            <td>${i+1}</td><td style="color:var(--gs-blue);font-weight:700">${s.ticker}</td><td>${s.name}</td>
    ...
    function showDetail(s) {
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
            <div><span style="font-size:1.4rem;font-weight:800;color:var(--gs-blue)">${s.ticker}</span>
            <span style="margin-left:0.75rem;color:var(--gs-text-dim)">${s.name}</span>
            <span style="margin-left:0.75rem;font-size:0.82rem;color:var(--gs-text-dim)">${s.sector} | ${s.industry}</span></div>
